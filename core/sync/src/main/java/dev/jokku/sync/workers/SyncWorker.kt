package dev.jokku.sync.workers

import android.content.Context
import dev.jokku.aggregate.data.sync.Synchronizer.sync
import dev.jokku.common.network.di.NewsDispatchers
import dev.jokku.data.sync.SyncConstraints
import dev.jokku.data.sync.syncForegroundInfo
...
class SyncWorker @dagger.assisted.AssistedInject constructor(
    @dagger.assisted.Assisted private val appContext: Context,
    @dagger.assisted.Assisted private val workerParameters: androidx.work.WorkerParameters,
    private val preferencesRepository: dev.jokku.data.repository.PreferencesRepository,
    private val newsRepository: dev.jokku.data.repository.NewsRepository,
    @dev.jokku.common.network.di.Dispatcher(dev.jokku.common.network.di.NewsDispatchers.IO) private val ioDispatcher: CoroutineDispatcher
) : androidx.work.CoroutineWorker(appContext, workerParameters),
    dev.jokku.aggregate.data.sync.Synchronizer {

    //Setting foreground notification in case of expedited work request
    override suspend fun getForegroundInfo(): androidx.work.ForegroundInfo =
        appContext.syncForegroundInfo()

    override suspend fun doWork(): androidx.work.ListenableWorker.Result = withContext(ioDispatcher) {
        androidx.tracing.traceAsync("Sync", 0) {
            val syncedSuccessfully = awaitAll(
                async { newsRepository.sync() }
            ).all { it }

            if (syncedSuccessfully) androidx.work.ListenableWorker.Result.success()
    ...
    companion object {
        fun startUpSyncWork() = androidx.work.OneTimeWorkRequestBuilder<DelegatingWorker>()
            ...
            .setConstraints(dev.jokku.data.sync.SyncConstraints)
