...
private fun Context.syncWorkNotification(): Notification {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        val channel = NotificationChannel(
            ...
            getString(dev.jokku.aggregate.R.string.sync_notification),
            NotificationManager.IMPORTANCE_DEFAULT
        ).apply {
            description = getString(dev.jokku.aggregate.R.string.foreground_task_for_syncing_news)
        }
    ...
    return NotificationCompat.Builder(this, SyncNotificationChannelId)
        .setSmallIcon(dev.jokku.aggregate.R.drawable.ic_aggregate_image_logo)
        .setContentTitle(getString(dev.jokku.aggregate.R.string.welcome_to_aggregate))
